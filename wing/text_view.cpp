...
void TextView::ExtendSelection(const soul::ast::SourcePos& start, const soul::ast::SourcePos& end)
{
...
void TextView::OnMouseDown(MouseEventArgs& args)
{
    ...
    if (args.buttons == MouseButtons::lbutton && !lines.empty())
    {
        ...
        if (line >= 1 && line <= lines.size())
        {
            ...
            mouseSelectionStart = soul::ast::SourcePos(line, column);
...
void TextView::OnMouseMove(MouseEventArgs& args)
{
    ...
    if (MouseExtendSelection())
    {
        ...
        {
            FixColumn(column, line);
            mouseSelectionEnd = soul::ast::SourcePos(line, column);
...
void TextView::OnKeyDown(KeyEventArgs& args)
{
    ...
    if (!args.handled)
    {
        ...
        {
            soul::ast::SourcePos start(CaretLine(), CaretColumn());
            soul::ast::SourcePos end(CaretLine(), 1);
        ...
        {
            soul::ast::SourcePos start(CaretLine(), CaretColumn());
            soul::ast::SourcePos end(CaretLine(), GetLineLength(lineNumber) + 1);
            ExtendSelection(start, end);
            columnNumber = GetLineLength(lineNumber) + 1 + LineNumberFieldLength();
        ...
        {
            ResetSelection();
            soul::ast::SourcePos start(1, 1);
            soul::ast::SourcePos end(static_cast<int>(lines.size() + 1), 1);
            ExtendSelection(start, end);
            InvalidateLines(selection.start.line, selection.end.line);
            args.handled = true;
            break;
...
void TextView::OnMouseDoubleClick(MouseEventArgs& args)
{
    ...
    ResetMouseExtendSelection();
    mouseSelectionStart = soul::ast::SourcePos();
    mouseSelectionEnd = soul::ast::SourcePos();
    SetFocus();
    ResetSelection();
    int line = 0;
    int column = 0;
    ...
    soul::ast::SourcePos start(line, column);
    c = GetCharAt(line, column);
    while (!util::IsWhiteSpace(c) && !IsWordSeparator(c))
    {
        if (!Next(line, column))
    ...
    soul::ast::SourcePos end(line, column);
    ExtendSelection(start, end);
